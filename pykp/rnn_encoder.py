...
class RNNEncoderTG(nn.Module):
    ...
    def forward(self, src, src_lens, src_mask, title, title_lens, title_mask):
        ...
        src_embed = self.embedding(src)  # [batch, src_len, embed_size]
        packed_src_embed = nn.utils.rnn.pack_padded_sequence(src_embed, src_lens, batch_first=True,enforce_sorted=False)
        ...
        packed_sorted_title_embed = nn.utils.rnn.pack_padded_sequence(sorted_title_embed, sorted_title_lens_tensor.tolist(), batch_first=True,enforce_sorted=False)
        ...
        # final merge layer
        packed_src_context = nn.utils.rnn.pack_padded_sequence(src_context_dropouted, src_lens, batch_first=True,enforce_sorted=False)
